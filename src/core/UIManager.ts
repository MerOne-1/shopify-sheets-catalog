...
class UIManager {
  ...
  showConfirmation(title, message) {
    const response = this.ui.alert(title, message, this.ui.ButtonSet.YES_NO);
  ...
  showInputDialog(title, prompt) {
    const response = this.ui.prompt(title, prompt, this.ui.ButtonSet.OK_CANCEL);
  ...
  showChoiceDialog(title, message, choices) {
    ...
    const response = this.ui.prompt(title, fullMessage, this.ui.ButtonSet.OK_CANCEL);
    
    if (response.getSelectedButton() === this.ui.Button.OK) {
      const choiceIndex = parseInt(response.getResponseText()) - 1;
  ...
  highlightErrors(sheetName, errorRanges) {
    ...
    errorRanges.forEach(range => {
      try {
        const cellRange = sheet.getRange(range.row, range.col);
   ...
   */
  showSetupWizard() {
    const message = `Welcome to Shopify Sheets Catalog Management!
